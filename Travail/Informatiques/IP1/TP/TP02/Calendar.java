public class Calendar {
	...
	public static int daysInMonth(int month, boolean inLeapYear) {
		// Modifier le code ci-dessous
		int day = 0;
		if (month == 2 && inLeapYear == true){
			day = 29; 
		}
		else if (month == 2 && inLeapYear == false){
			day= 28;
		}
	...
	public static int julianToCounter(int year, int month, int day){
		...
		for (int i=1; i<year; i++){
			jour1 = jour1 + daysInYearJulian(i); 
		}
		for (int j=1; j<month; j++){
			jour2 = jour2 + daysInMonth(j, isLeapYearJulian(year));
		}

		int comp = jour1 + jour2 + day - 2;
		return comp;
	}
	/*EXERCICE 7*/
	// A completer
	...
	/*EXERCICE 8*/
	//Ca ne change pas car on précise déjà quand si l'année est bisextile ou pas donc aucun des mois vous changer.
	...
	public static int gregorianToCounter(int year, int month, int day) {
		...
		int jour2 = 0;
		for (int i=1; i<year; i++){
			jour1 = jour1 + daysInYearGregorian(i); 
		}
		for (int j=1; j<month; j++){
			jour2 = jour2 + daysInMonth(j, isLeapYearGregorian(year));
		}
		conv = day + jour1 + jour2;
		return conv;
	}
